...
$PLUGIN_SHORT = 'ssa_nots';

$option_group = $PLUGIN_SHORT.'_theme_option_group';
$option_name = $PLUGIN_SHORT.'_theme_options';

// Load stylesheet and jscript
add_action('admin_init', 'nots_admin_add_init');

function nots_admin_add_init() {
	
	wp_enqueue_style("notsCss",  WP_PLUGIN_URL . '/' . plugin_basename( dirname( __FILE__ ) ) . "/nots-options.css", false, "1.0", "all");

	wp_enqueue_script("notsScript",  WP_PLUGIN_URL . '/' . plugin_basename( dirname( __FILE__ ) ) ."/nots-options.js", false, "1.0");

}

// Create custom settings menu
add_action('admin_menu', 'nots_admin_create_menu');

function nots_admin_create_menu() {
	//global $themename;
	//create new top-level menu
	add_options_page( __( $PLUGIN.' Options' ), __( $PLUGIN . ' Options' ), 'manage_options', basename(__FILE__), 'ssa_nots_settings_page' );
}

// Register settings
add_action( 'admin_init', 'register_nots_settings' );

function register_nots_settings() {
   global $themename, $shortname, $version, $ssa_nots_options, $option_group, $option_name;
...
$ssa_nots_options = array (

array("name" => __('General', $PLUGIN_SHORT),

		"type" => "section"),

array("name" => __('Setup your notifications.',$PLUGIN_SHORT),
...
function ssa_nots_settings_page() {
   global $themename, $shortname, $version, $ssa_nots_options, $option_group, $option_name;
?>
...
<?php // Delete Settings
global $wpdb, $themename, $shortname, $version, $ssa_nots_options, $option_group, $option_name;
...
<?php foreach ($ssa_nots_options as $value) {
